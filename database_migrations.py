...
# Render (and some other hosts) hand out the connection string starting with
# "postgres://", but SQLAlchemy 2.x requires the "postgresql://" scheme.
# This one line makes the same code work both locally and in the cloud.
if DATABASE_URL.startswith("postgres://"):
    DATABASE_URL = DATABASE_URL.replace("postgres://", "postgresql://", 1)

# On a managed host the database already exists and your user usually cannot
# CREATE DATABASE. Set SKIP_DB_CREATE=1 (done in render.yaml) to skip that step
# and go straight to creating the tables.
...
if __name__ == "__main__":
    ...
    print("\n1. Checking database...")
    if SKIP_DB_CREATE:
        print("  ↳ SKIP_DB_CREATE=1 — managed host already provides the database, "
              "skipping CREATE DATABASE.")
    else:
        create_database()
